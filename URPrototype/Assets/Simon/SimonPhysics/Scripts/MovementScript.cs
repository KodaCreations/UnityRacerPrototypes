...
public class MovementScript : NetworkBehaviour {
	...
	void InputHandler()
    {
        ...
        if (Input.GetKey(KeyCode.A))
        {
            ...
            //rb.AddTorque(new Vector3(0, -rotationSpeed * Time.deltaTime, 0), ForceMode.Acceleration);
        }
    ...
    void HoverHandler()
    {
        ...
        if (Physics.Raycast(ray, out hit, 10.0f))
        {
            ...
            wantedTrackPos = hit.point + hit.normal * hoverHeight;//Vector3.Slerp(transform.position, hit.point + hit.normal * hoverHeight, Time.deltaTime * correctionPosDelay);
            ...
            //rb.position = hit.point + hit.normal * hoverHeight;
            ...
            //rb.rotation = grndTilt * Quaternion.Euler(0, rb.rotation.eulerAngles.y, 0);
    ...
    void SteeringGroundBehavior()
    {
        ...
        // Banking diffrent amount depending on current bank
        #region Banking
        if (steeringForce > 0)
        {
    ...
    [Command]
    void CmdSendInputHandler(float steeringInput)
    {
	...
	void FixedUpdate ()
    {
        ...
        {
            InputHandler();
            CmdSendInputHandler(steeringForce);
        }
